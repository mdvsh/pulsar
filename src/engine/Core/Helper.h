//
// Created by Madhav Shekhar Sharma on 4/9/24.
//

#ifndef PULSAR_SRC_ENGINE_CORE_HELPER_H_
#define PULSAR_SRC_ENGINE_CORE_HELPER_H_
...
/* WARNING : You may need to adjust the following include paths if your headers
 * / file structures are different. */
/* Here is the instructor solution folder structure (if we make $(ProjectDir) a
 * include directory, these paths are valid. */
...
/* The Helper class contains mostly static functions / data, and doesn't need to
 * be instanced. */
/* Call the public static functions below via Helper::<function>() */
class Helper {
 public:
  /* Turn RECORDING_MODE on to record inputs as you play. */
  /* The input file may be fed back in to replay your game session (autograder
   * does this). */
  inline static const bool RECORDING_MODE = false;
  inline static const char* USER_INPUT_FILENAME = "sdl_user_input.txt";

  /* The Helper.h function works differently (and thus your program works
   * differently) */
  /* Depending on whether or not an autograder is testing it. */
  inline static bool _autograder_mode = false;

  /* One way the autograder gauges success is by comparing your "frames"
   * (renderings) to */
  /* that of a staff solution program fed the exact same input. These are placed
   * into a "frames" folder. */
  ...
  static SDL_Window* SDL_CreateWindow498(const char* title,
                                         int x,
                                         int y,
                                         int w,
                                         int h,
  ...
  static SDL_Renderer* SDL_CreateRenderer498(SDL_Window* window,
                                             int index,
                                             Uint32 flags) {
    if (IsAutograderMode())
      flags &= ~SDL_RENDERER_PRESENTVSYNC;  // VSync is disabled to let frames
                                            // render faster in the autograder.
  ...
  /* Wrapper that will inject input events into the SDL Event Queue if a user
   * input file is found */
  ...
  static void SDL_RenderPresent498(SDL_Renderer* renderer) {
    if (renderer == nullptr) {
      std::cout << "ERROR : The renderer pointer passed to "
                   "Helper::SDL_RenderPresent498() is a nullptr."
                << std::endl;
      exit(0);
    }

    if (input_status == NOT_INITIALIZED) {
      std::cout << "ERROR : Please do not attempt to render "
                   "(Helper::SDL_RenderPresent498()) before you've entered the "
                   "game loop (IE, begun calling Helper::SDL_PollEvent498()."
                << std::endl;
      exit(0);
    }
    ...
    if (RECORDING_MODE || _autograder_mode) {
      if (!initialized) {
        ...
        saving_surface = SDL_CreateRGBSurfaceWithFormat(0, width, height, 24,
      ...
      /* Read the current renderer's data and persist it as a .bmp file to disk
       * (BMP format is fast-to-write compared to PNG). */
      if (SDL_RenderReadPixels(renderer, NULL, SDL_PIXELFORMAT_RGB24,
                               saving_surface->pixels,
                               saving_surface->pitch) != 0) {
      ...
      std::string output_file_path =
          frame_directory_relative_path + "/" + output_file_name;
    ...
    if (!_autograder_mode)
      ::SDL_RenderPresent(
          renderer);  // The autograder doesn't need to render to window.
    SDL_Delay();
    frame_number++;
  }

  /* FEATURE : Render Logger */
  /* Create a "RENDERLOGGER" environmental variable, run your engine, and check
   * render_logger.txt. */
  /* Use to compare to test case render_logger.txt files to see what goes wrong
   * in your render. */
  ...
  static void CheckForRenderLoggerInit() {
    ...
    if (render_logger_mode == RL_NOT_INITIALIZED) {
      if (IsLoggingMode()) {
        ...
        std::ofstream file("render_logger.txt",
                           std::ios::out);  // delete the file if it exists.
        render_logging_file.open("render_logger.txt", std::ios::app);

        if (!render_logging_file.is_open()) {
          std::cerr << "Error : Failed to open render_logger.txt for writing."
                    << std::endl;
        }

        render_logging_file << "== RENDER LOGGER ==" << std::endl;
        render_logging_file << "Study the following SDL_RenderCopyEx498() "
                               "calls to debug render-related issues."
                            << std::endl;
        render_logging_file
            << "Enable render logger mode on your computer by setting the "
               "RENDERLOGGER environmental variable."
            << std::endl;
        render_logging_file << "frame:actor_id:actor_name" << std::endl
                            << std::endl;
      } else {
  ...
  static void SDL_RenderCopyEx498(int actor_id,
    ...
    if (render_logger_mode == RL_ENABLED) {
      ...
      render_logging_file << GetFrameNumber() << ":" << actor_id << ":"
                          << actor_name << " dstrect " << dstrect->x << " "
                          << dstrect->y << " " << dstrect->w << " "
                          << dstrect->h << " angle " << angle << " center "
                          << center->x << " " << center->y << " flip " << flip
                          << " renderscale " << x_scale << " " << y_scale
                          << std::endl;
    }
  }

 private:
  static inline std::unordered_map<int, std::queue<SDL_Event>>
      frame_to_user_input;
  ...
  static void SDL_ConsiderInputFile() {
    ...
    if (RECORDING_MODE && input_status != INPUT_FILE_PRESENT &&
        !_autograder_mode) {
      /* Peek at incoming events so we may record them before the student code
       * consumes them. */
      SDL_PumpEvents();
      SDL_Event incoming_events[100];
      int num_events = SDL_PeepEvents(incoming_events, 100, SDL_PEEKEVENT,
      ...
      for (int i = 0; i < num_events; i++) {
        ...
        if (event_type == SDL_KEYUP || event_type == SDL_KEYDOWN ||
            ...
            event_type == SDL_MOUSEBUTTONUP || event_type == SDL_MOUSEWHEEL ||
            event_type == SDL_QUIT)
      ...
      if (relevant_events.size() > 0) {
        ...
        for (int i = 0; i < relevant_events.size(); i++) {
          ...
          /* Each event could be of multiple types, with differing numbers of
           * parameters. */
  ...
  static bool IsAutograderMode() { return IsEnvVariableSet("AUTOGRADER"); }

  static bool IsLoggingMode() { return IsEnvVariableSet("RENDERLOGGER"); }

  /* The engine will aim for 60fps (16ms per frame) during a normal play
   * session. */
  /* If the engine detects it is being autograded, it will run as fast as
   * possible. */
  static void SDL_Delay() {
    if (_autograder_mode) {
      //::SDL_Delay(1); Don't bother delaying at all. Gotta go fast when
      //: autograding.
    } else {
    ...
    current_frame_start_timestamp =
        SDL_GetTicks();  // Record start time of the frame
  }
  ...
  static void LoadSDLEventsFromInputFile() {
    ...
    while (std::getline(infile, line)) {
      ...
      while (std::getline(iss, eventStr, ';')) {
        ...
        // (may be needed for stoi to work on osx / linux when
        // sdl_user_input.txt has been authored on windows) (depends on how
        // student chooses to download the test cases)
        event_type_string.erase(std::remove(event_type_string.begin(),
                                            event_type_string.end(), '\r'),
        ...
        if (event_type == SDL_KEYUP || event_type == SDL_KEYDOWN) {
          ...
          fabricated_sdl_event.button.button =
              static_cast<Uint8>(std::stoi(mouse_button_index_str));
        } else if (event_type == SDL_MOUSEWHEEL) {
          ...
          fabricated_sdl_event.wheel.preciseY =
              std::stof(mouse_wheel_movement_str);
        }
...
/* Disable usage of several non-wrapped, vanilla SDL functions via the macro
 * redefinition technique. */
/* By forcing usage of "wrapper" / "helper" functions, the course staff gains
 * control of program needed for autograder. */
#define SDL_CreateWindow \
  DO_NOT_USE_VANILLA_SDL_CreateWindow_USE_HELPER_SDL_CreateWindow498_Instead
#define SDL_CreateRenderer \
  DO_NOT_USE_VANILLA_SDL_CreateRenderer_USE_HELPER_SDL_CreateRenderer498_INSTEAD
#define SDL_PollEvent \
  DO_NOT_USE_SDL_PollEvent_DIRECTLY_USE_HELPER_SDL_PollEvent498_INSTEAD
#define SDL_RenderPresent \
  DO_NOT_USE_VANILLA_SDL_RenderPresent_USE_HELPER_SDL_RenderPresent498_INSTEAD
#define SDL_RenderCopyEx \
  DO_NOT_USE_VANILLA_SDL_RenderCopyEx_USE_HELPER_SDL_RenderCopyEx498_INSTEAD


#endif  // PULSAR_SRC_ENGINE_CORE_HELPER_H_

...
#ifndef PULSAR_SRC_ENGINE_CORE_AUDIOHELPER_H_
#define PULSAR_SRC_ENGINE_CORE_AUDIOHELPER_H_
...
class AudioHelper {
 public:
  ...
  static inline int Mix_PlayChannel498(int channel,
                                       Mix_Chunk* chunk,
                                       int loops) {
    std::cout << "(Mix_PlayChannel498(" << channel << ",?," << loops
              << ") called on frame " << Helper::GetFrameNumber() << ")"
              << std::endl;
  ...
  static inline int Mix_OpenAudio498(int frequency,
                                     Uint16 format,
                                     int channels,
                                     int chunksize) {
    if (!IsAutograderMode())
  ...
  [[maybe_unused]] static inline void Mix_Pause498(int channel) {
    std::cout << "(Mix_Pause498(" << channel << ") called on frame "
              << Helper::GetFrameNumber() << ")" << std::endl;

    if (!IsAutograderMode())
      Mix_Pause(channel);
  }
  [[maybe_unused]] static inline void Mix_Resume498(int channel) {
    std::cout << "(Mix_Resume498(" << channel << ") called on frame "
              << Helper::GetFrameNumber() << ")" << std::endl;
  ...
  static inline int Mix_HaltChannel498(int channel) {
    std::cout << "(Mix_HaltChannel498(" << channel << ") called on frame "
              << Helper::GetFrameNumber() << ")" << std::endl;
  ...
  static inline int Mix_Volume498(int channel, int volume) {
    std::cout << "(Mix_Volume498(" << channel << "," << volume
              << ") called on frame " << Helper::GetFrameNumber() << ")"
              << std::endl;
  ...
  [[maybe_unused]] static inline void Mix_CloseAudio498(void) {
    if (!IsAutograderMode())
      Mix_CloseAudio();
  }
 ...
 private:
  ...
  /* It's best for everyone if the autograder doesn't actually play any audio
   * (it will still check that you try though). */
  /* Imagine poor Professor Kloosterman hearing the autograder all day as it
   * spams various sounds in my office. */
...
#endif  // PULSAR_SRC_ENGINE_CORE_AUDIOHELPER_H_
